############################################################## 
# Date: 20/01/16
# Name: plot_coast_boxplot2BULK.py
# Author: Alek Petty
# Description: Script to plot bulk coastline boxplots
...
def get_ridge_dists_precentsBULK(region_str):
	coastdatapath = datapath+'COAST/BULK/'
	z_c_int_mayears=[]
	volume_mayears=[]
	z_c_int_maALL=[]
	volume_maALL=[]
	...
	for x in xrange(size(years)):
		...
		z_c_int_maT=load(coastdatapath+'z_c_int_ma'+region_str+str(years[x]+start_year)+'.txt')
		z_c_int_mayears.append(z_c_int_maT)
		z_c_int_maALL.extend(z_c_int_maT)
		volume_maT=load(coastdatapath+'volume_ma'+region_str+str(years[x]+start_year)+'.txt') 
		volume_mayears.append(volume_maT)
		volume_maALL.extend(volume_maT)

	z_c_int_maALL=array(z_c_int_maALL)
	volume_maALL=array(volume_maALL)
	sort_indices = np.argsort(z_c_int_maALL, axis=0)
	z_c_int_maALL=z_c_int_maALL[sort_indices]
	volume_maALL=volume_maALL[sort_indices]

	volume_maALL_sections=[]
	...
	for bin_num in xrange(num_bins):
		...
		volume_maALL_sections.append(volume_maALL[dist_idxs])


	return volume_maALL_sections, num_bins

def get_ridge_dists_precentsBULKYEARS(region_str,x):
	coastdatapath = datapath+'COAST/BULK/'
	...
	volume_maT=volume_maT[sort_indices]

	volume_maT_sections=[]

	for bin_num in xrange(num_bins):
		dist_idxs=where((z_c_int_maT>(bin_num*bin_width))&(z_c_int_maT<((bin_num+1)*bin_width)))
		volume_maT_sections.append(volume_maT[dist_idxs])


	return volume_maT_sections, num_bins
...
height_maALLCA_sections, num_binsCA = get_ridge_dists_precentsBULK('CA')
height_maALLBC_sections, num_binsBC = get_ridge_dists_precentsBULK('BC')

height_maALLCA_sections_years=[]
height_maALLBC_sections_years=[]
for x in xrange(num_years):
	height_maALLCA_sections_year, num_binsCA = get_ridge_dists_precentsBULKYEARS('CA', x)
	height_maALLBC_sections_year, num_binsBC = get_ridge_dists_precentsBULKYEARS('BC', x)
...
ax1.set_ylabel(label_str, labelpad=3)
...
ax2.set_xlim(0, 9)
ax2.set_ylim(0, 0.6)
ax2.set_ylabel(label_str, labelpad=3)
...
savefig(figpath+'figure11.pdf', dpi=300)
