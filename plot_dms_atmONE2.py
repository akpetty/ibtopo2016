############################################################## 
# Date: 20/01/16
# Name: plot_dms_atmONE2.py
# Author: Alek Petty
# Description: Script to plot another detection case study
...
def get_atm_from_dms(res):
	...
	for j in xrange(size(atm_files)):
		atm_time = float(atm_files[j][69:75])
		#print atm_time
...
def plot_6plot_thresh(dms_plot=0):
	...
	ax2 = subplot(322)
	ax2.annotate('(b) Raw DMS + ATM' , xy=(0.95, 0.9), textcoords='axes fraction', color='k', horizontalalignment='right', verticalalignment='middle')
	...
	#elevation2d_ma_1d = ma.compressed(elevation_ma.ravel())
	...
	#axhline(y=level_elevu, linestyle='--', color='b')
	#axvline(x=pwidth*min_index, ymax = level_elevl, linestyle='--', color='b')
	#axvline(x=pwidth*(min_index+1), ymax = level_elevu, linestyle='--', color='b')
	axhline(thresh, color='r')
	ax3.annotate('Sail height threshold' , xy=(10, thresh), xycoords='data', color='r', horizontalalignment='left', verticalalignment='bottom')
	...
	#plot([mid_percent, mid_percent], [level_elevu, thresh], '-g')
	#annotate ('', (mid_percent, level_elevu), (mid_percent, thresh), arrowprops={'arrowstyle':'<->'})
	#arrow(mid_percent, level_elevu, 0, min_ridge_height, fc="k", ec="k",head_width=0.05, head_length=0.1)
	ax3.annotate(str(min_ridge_height)+' m' , xy=(mid_percent-15, level_elevl + (thresh-level_elevu)*0.7), color='k', horizontalalignment='left', verticalalignment='bottom')
	...
	norm = apy.MidpointNormalize(midpoint=0)
	ax4 = subplot(324)
	ax4.annotate('(d) Gridded ('+str(xy_res)+' m) ATM' , xy=(0.95, 0.9), textcoords='axes fraction', color='k', horizontalalignment='right', verticalalignment='middle')
	
	im4 = pcolormesh(xx2d, yy2d, elevation2d-level_elev, norm=norm,vmin=-1.01, vmax=2.01, cmap = cm.RdBu_r)
	#
	im4.set_clim(-1.01, 2.01)
	...
	if (found_ridges==1):
		...
		#minLABEL = 0
		#maxLABEL = int(np.amax(label_im_mask))
		im51 = contour(xx2d, yy2d, label_im, np.arange(0, num_ridges+1), colors='k', linewidths=1)

	#ax5.annotate('Level ice \n and ridged ice \n (ridge areas>100m2)' , xy=(0.7, 0.93), textcoords='axes fraction', color='k', horizontalalignment='middle', verticalalignment='middle')
	...
	#minvalR = round(np.amin(ridge_height_mesh), 1)
	#maxvalR = round(np.amax(ridge_height_mesh), 1)
	#if ((maxvalR-minvalR)<0.1):
	#	minvalR=minvalR-0.1
	#	maxvalR=maxvalR+0.1
	...
	cax1 = fig.add_axes([0.58, 0.39, 0.02, 0.08])
	...
	if (found_big_ridge==1):
		...
		cbar3.set_label('Ridge number', labelpad=30, rotation=0)
...
#label_imB = ro.get_labelsBIGNOWATER(elevation2d_ridge_maL, num_points_req)
...
blobs_log = blob_log(elevation2d_ridge_ma)
